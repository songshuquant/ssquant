...
if __name__ == "__main__":
    ...
    # ==================== 运行模式选择 ====================
    RUN_MODE = RunMode.SIMNOW      # 可选: SIMNOW, REAL_TRADING
    ...
    if RUN_MODE == RunMode.SIMNOW:
        ...
        config = get_config(RUN_MODE,
            ...
            # -------- 合约配置 --------
            symbol='au2602',                  # 黄金合约（流动性好，适合测试）
            kline_period='tick',              # K线周期
            
            # -------- 交易参数 --------
            # ⚠️ price_tick 必须与合约匹配: 黄金=0.02, 螺纹钢=1, 铜=10
            price_tick=0.02,                  # 黄金最小变动价位
            # offset_ticks 控制委托价格偏移: 委托价 = 基准价 + offset_ticks × price_tick
            # 负数=限价委托(不易成交), 正数=超价委托(快速成交)
            order_offset_ticks=-10,           # 测试用负偏移，降低成交概率
            
            # -------- 回调模式配置 --------
            enable_tick_callback=True,        # True=每个TICK触发策略
            
            # -------- 历史数据配置 --------
            preload_history=True,             # 预加载历史K线
            history_lookback_bars=50,         # 预加载数量
            adjust_type='1',                  # 复权类型

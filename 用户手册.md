# SSQuant 用户手册

> 完整的使用教程

## 📖 目录

1. [框架概述](#框架概述)
2. [安装配置](#安装配置)
3. [快速开始](#快速开始)
4. [账户配置](#账户配置)
5. [策略编写](#策略编写)
6. [回测模式](#回测模式)
7. [SIMNOW模拟](#simnow模拟)
8. [实盘部署](#实盘部署)
9. [常见问题](#常见问题)

---

## 框架概述

### 核心理念

**一次编写，多处运行** - 同一份策略代码可以在回测、模拟、实盘三种环境下运行。

### 三种运行模式

| 模式 | 枚举值 | 说明 | 适用场景 |
|------|--------|------|---------|
| **回测** | `RunMode.BACKTEST` | 历史数据回测 | 策略开发和验证 |
| **模拟** | `RunMode.SIMNOW` | CTP模拟盘 | 实盘前测试 |
| **实盘** | `RunMode.REAL_TRADING` | 真实CTP交易 | 正式交易 |

### 数据流

```
回测模式：历史数据 → 回测引擎 → 策略 → 模拟成交
SIMNOW：  CTP行情 → 实时K线合成 → 策略 → 模拟下单
实盘模式：CTP行情 → 实时K线合成 → 策略 → 真实下单
```

---

## 安装配置

### 系统要求

- **Python**: 3.9 ~ 3.14
- **操作系统**: Windows 10+ (CTP仅支持Windows)
- **内存**: 4GB+

### 安装方式

```bash
# 方式1：从 PyPI 安装（推荐）
pip install ssquant

# 方式2：从源码安装
git clone https://github.com/songshuquant/ssquant
cd ssquant
pip install -e .
```

### 安装依赖（可选）

```bash
# 机器学习策略需要
pip install scikit-learn joblib statsmodels
```

### 目录结构

```
ssquant/                    # 项目根目录
├── ssquant/               # 核心包（pip install后可用）
│   ├── api/               # 策略API
│   ├── backtest/          # 回测引擎  
│   ├── config/            # 配置（账户配置在此）
│   ├── data/              # 数据管理
│   ├── pyctp/             # CTP接口封装
│   ├── ctp/               # CTP二进制文件
│   └── indicators/        # 技术指标
├── examples/              # 示例策略（19个）
├── data_cache/            # 数据缓存（运行时生成）
└── live_data/             # 实盘数据落盘（运行时生成）
```

---

## 快速开始

### 第一个策略（3分钟）

**步骤1：创建策略文件 `my_first_strategy.py`**

```python
from ssquant.api.strategy_api import StrategyAPI
from ssquant.backtest.unified_runner import UnifiedStrategyRunner, RunMode
from ssquant.config.trading_config import get_config

def my_strategy(api: StrategyAPI):
    """简单的MA策略"""
    close = api.get_close()
    
    if len(close) < 20:
        return
    
    ma20 = close.rolling(20).mean()
    pos = api.get_pos()
    
    # 价格在均线上方做多
    if close.iloc[-1] > ma20.iloc[-1] and pos <= 0:
        if pos < 0:
            api.buycover(order_type='next_bar_open')
        api.buy(volume=1, order_type='next_bar_open')
        api.log("买入信号")
    
    # 价格在均线下方做空
    elif close.iloc[-1] < ma20.iloc[-1] and pos >= 0:
        if pos > 0:
            api.sell(order_type='next_bar_open')
        api.sellshort(volume=1, order_type='next_bar_open')
        api.log("卖出信号")

if __name__ == "__main__":
    # 回测配置
    config = get_config(
        mode=RunMode.BACKTEST,
        symbol='rb888',               # 主力连续合约
        start_date='2025-01-01',
        end_date='2025-11-30',
        kline_period='1h',            # 1小时K线
        price_tick=1.0,               # 最小变动价位
        contract_multiplier=10,       # 合约乘数
        slippage_ticks=1,             # 滑点
    )
    
    # 运行
    runner = UnifiedStrategyRunner(mode=RunMode.BACKTEST)
    runner.set_config(config)
    results = runner.run(strategy=my_strategy)
```

**步骤2：运行**

```bash
python my_first_strategy.py
```

**完成！** 你会看到回测结果。

---

## 账户配置

### 配置文件位置

`ssquant/config/trading_config.py`

### 数据API配置

如果需要自动获取历史数据：

```python
# 编辑 trading_config.py
API_USERNAME = "你的用户名"    # quant789.com 账号
API_PASSWORD = "你的密码"
```

### SIMNOW账户配置

```python
ACCOUNTS = {
    'simnow_default': {
        # 账户认证 (必填)
        'investor_id': '你的SIMNOW账号',   # simnow.com.cn 注册
        'password': '你的密码',
        'server_name': '电信1',             # 服务器: 电信1/电信2/移动/TEST
        
        # 交易参数
    'price_tick': 1.0,                  # 最小变动价位
    'order_offset_ticks': 5,            # 超价跳数 (对于市价单/next_bar_open有效)
    
    # 智能算法交易 (Algo Trading)
    'algo_trading': True,               # 启用算法交易
    'order_timeout': 10,                # 订单超时时间(秒)
    'retry_limit': 3,                   # 最大重试次数
    'retry_offset_ticks': 5,            # 重试时的超价跳数
    
    # 数据配置
    'preload_history': True,            # 预加载历史K线
        'history_lookback_bars': 100,       # 预加载数量
        'adjust_type': '1',                 # 复权: 0不复权, 1后复权
        
        # 回调配置
        'enable_tick_callback': False,      # TICK回调模式
        
        # 数据保存
        'save_kline_csv': False,
        'save_kline_db': False,
        'save_tick_csv': False,
        'save_tick_db': False,
    },
}
```

### 实盘账户配置

```python
ACCOUNTS = {
    'real_default': {
        # 账户认证 (从期货公司获取)
        'broker_id': '期货公司代码',
        'investor_id': '资金账号',
        'password': '交易密码',
        'md_server': 'tcp://xxx.xxx.xxx.xxx:port',  # 行情服务器
        'td_server': 'tcp://xxx.xxx.xxx.xxx:port',  # 交易服务器
        'app_id': '应用ID',
        'auth_code': '授权码',
        
        # 其他参数同 simnow_default
    },
}
```

### 使用账户

```python
# SIMNOW
config = get_config(
    mode=RunMode.SIMNOW,
    account='simnow_default',   # 使用配置好的账户
    symbol='rb2601',
)

# 实盘
config = get_config(
    mode=RunMode.REAL_TRADING,
    account='real_default',     # 使用配置好的账户
    symbol='rb2601',
)
```

---

## 策略编写

### 策略函数结构

```python
def my_strategy(api: StrategyAPI):
    """
    策略函数 - 每根K线执行一次
    
    Args:
        api: 策略API对象
    """
    # 1. 获取数据
    close = api.get_close()
    
    # 2. 数据验证
    if len(close) < 20:
        return
    
    # 3. 计算指标
    ma20 = close.rolling(20).mean()
    
    # 4. 获取持仓
    pos = api.get_pos()
    
    # 5. 交易信号
    if close.iloc[-1] > ma20.iloc[-1]:
        if pos <= 0:
            api.buy(volume=1, order_type='next_bar_open')
```

### 初始化函数（可选）

```python
def initialize(api: StrategyAPI):
    """策略初始化 - 运行前执行一次"""
    api.log("策略初始化")
    fast = api.get_param('fast_ma', 5)
    slow = api.get_param('slow_ma', 20)
    api.log(f"参数: 快线={fast}, 慢线={slow}")

# 运行时传入
runner.run(
    strategy=my_strategy,
    initialize=initialize,
    strategy_params={'fast_ma': 5, 'slow_ma': 20}
)
```

### 常用API

```python
# ===== 数据获取 =====
api.get_close()          # 收盘价序列
api.get_open()           # 开盘价序列
api.get_high()           # 最高价序列
api.get_low()            # 最低价序列
api.get_volume()         # 成交量序列
api.get_klines()         # 完整K线DataFrame
api.get_price()          # 当前价格
api.get_datetime()       # 当前时间
api.get_idx()            # 当前K线索引

# ===== 持仓查询 =====
api.get_pos()            # 净持仓（正=多，负=空）
api.get_long_pos()       # 多头持仓
api.get_short_pos()      # 空头持仓
api.get_position_detail()# 详细持仓（今昨仓）

# ===== 交易操作 =====
api.buy(volume=1)        # 买入开仓
api.sell()               # 卖出平仓（平多）
api.sellshort(volume=1)  # 卖出开仓（开空）
api.buycover()           # 买入平仓（平空）
api.close_all()          # 全部平仓
api.reverse_pos()        # 反手

# ===== 参数和日志 =====
api.get_param('key', default)  # 获取参数
api.log("消息")                # 记录日志

# ===== TICK数据（实盘） =====
api.get_tick()           # 当前TICK
api.get_ticks(100)       # 最近100个TICK
```

详见 [API参考手册.md](API参考手册.md)

---

## 回测模式

### 基本配置

```python
from ssquant.backtest.unified_runner import UnifiedStrategyRunner, RunMode
from ssquant.config.trading_config import get_config

config = get_config(
    mode=RunMode.BACKTEST,
    
    # 合约配置
    symbol='rb888',               # 主力连续合约
    start_date='2025-01-01',      # 开始日期
    end_date='2025-11-30',        # 结束日期
    kline_period='1h',            # K线周期: 1m/5m/15m/30m/1h/4h/1d
    adjust_type='1',              # 复权: 0不复权, 1后复权
    
    # 合约参数
    price_tick=1.0,               # 最小变动价位
    contract_multiplier=10,       # 合约乘数
    slippage_ticks=1,             # 滑点跳数
    
    # 资金参数
    initial_capital=100000,       # 初始资金
    commission=0.0001,            # 手续费率
    margin_rate=0.1,              # 保证金率
)

runner = UnifiedStrategyRunner(mode=RunMode.BACKTEST)
runner.set_config(config)
results = runner.run(strategy=my_strategy)
```

### 多品种回测

```python
config = get_config(
    mode=RunMode.BACKTEST,
    start_date='2025-01-01',
    end_date='2025-11-30',
    
    data_sources=[
        {
            'symbol': 'rb888',
            'kline_period': '1h',
            'price_tick': 1.0,
            'contract_multiplier': 10,
        },
        {
            'symbol': 'i888',
            'kline_period': '1h',
            'price_tick': 0.5,
            'contract_multiplier': 100,
        },
    ],
)

# 策略中访问不同数据源
def multi_symbol_strategy(api):
    close_rb = api.get_close(index=0)  # rb
    close_i = api.get_close(index=1)   # i
    
    api.buy(volume=1, index=0)  # 在rb上交易
    api.buy(volume=1, index=1)  # 在i上交易
```

### 多周期回测

```python
config = get_config(
    mode=RunMode.BACKTEST,
    start_date='2025-01-01',
    end_date='2025-11-30',
    
    data_sources=[
        {'symbol': 'rb888', 'kline_period': '5m'},
        {'symbol': 'rb888', 'kline_period': '1h'},
    ],
)

# 策略中使用多周期
def multi_period_strategy(api):
    close_5m = api.get_close(index=0)   # 5分钟
    close_1h = api.get_close(index=1)   # 1小时
    
    # 用1小时判断趋势，5分钟进场
```

详见 [回测教程.md](回测教程.md)

---

## SIMNOW模拟

### 申请账号

访问 http://www.simnow.com.cn/ 注册免费账号

### 配置账户

编辑 `ssquant/config/trading_config.py`：

```python
ACCOUNTS = {
    'simnow_default': {
        'investor_id': '你的SIMNOW账号',
        'password': '你的密码',
        'server_name': '电信1',
        # ...
    },
}
```

### 运行策略

```python
config = get_config(
    mode=RunMode.SIMNOW,
    account='simnow_default',
    
    # 合约配置
    symbol='rb2601',              # 具体合约（不能用主连888）
    kline_period='1m',            # K线周期
    
    # 交易参数
    price_tick=1.0,               # 最小变动价位
    order_offset_ticks=5,         # 超价跳数（确保成交）
    
    # 智能算法交易
    algo_trading=True,            # 启用算法交易
    order_timeout=10,             # 10秒超时
    retry_limit=3,                # 重试3次
    retry_offset_ticks=5,         # 重试偏移
    
    # 历史数据
    preload_history=True,         # 预加载历史K线
    history_lookback_bars=100,    # 预加载数量
    adjust_type='1',              # 复权类型
    
    # 回调模式
    enable_tick_callback=False,   # False=K线驱动，True=TICK驱动
    
    # 数据保存
    save_kline_csv=True,          # 保存K线到CSV
    save_tick_csv=True,           # 保存TICK到CSV
)

runner = UnifiedStrategyRunner(mode=RunMode.SIMNOW)
runner.set_config(config)
runner.run(strategy=my_strategy)
```

### 服务器选择

| server_name | 说明 |
|------------|------|
| `'电信1'` | 电信用户优先 |
| `'电信2'` | 备用线路 |
| `'移动'` | 移动用户优先 |
| `'TEST'` | 测试服务器（周末可用） |

详见 [SIMNOW使用指南.md](SIMNOW使用指南.md)

---

## 实盘部署

### ⚠️ 安全警告

**实盘前必读：**

1. ✅ 在SIMNOW充分测试（至少1周）
2. ✅ 先用最小手数（1手）验证
3. ✅ 设置止损/止盈
4. ✅ 监控策略运行状态
5. ✅ 准备紧急停止方案

### 配置实盘账户

编辑 `ssquant/config/trading_config.py`：

```python
ACCOUNTS = {
    'real_default': {
        'broker_id': '期货公司代码',        # 如 '9999'
        'investor_id': '资金账号',
        'password': '交易密码',
        'md_server': 'tcp://xxx:port',     # 行情服务器
        'td_server': 'tcp://xxx:port',     # 交易服务器
        'app_id': 'AppID',
        'auth_code': '授权码',
        # ...
    },
}
```

### 运行实盘

```python
config = get_config(
    mode=RunMode.REAL_TRADING,
    account='real_default',
    symbol='rb2601',
    kline_period='1m',
    price_tick=1.0,
    order_offset_ticks=5,
    preload_history=True,
    history_lookback_bars=100,
)

runner = UnifiedStrategyRunner(mode=RunMode.REAL_TRADING)
runner.set_config(config)

try:
    runner.run(strategy=my_strategy)
except KeyboardInterrupt:
    runner.stop()
    print("策略已停止")
```

详见 [实盘部署指南.md](实盘部署指南.md)

---

### 7. 高级交易功能

#### 7.1 限价单与Maker策略
除了默认的K线成交和市价单，框架还支持发送纯限价单（Limit Order）：

```python
# 指定 order_type='limit' 并提供价格
api.buy(volume=1, order_type='limit', price=3500.0)

# 或者直接提供价格 (自动识别为限价单)
api.sell(volume=1, price=3600.0)
```

配合 **智能算法交易** 配置，可以实现 "先排队(Maker)，超时后吃单(Taker)" 的高级策略：
1. 策略发出限价单，挂在买一价上排队。
2. 设置 `order_timeout=5`，如果5秒内未成交，框架自动撤单。
3. 设置 `retry_offset_ticks=5`，撤单后框架自动以 "对手价+5跳" 发出新订单，确保成交。

#### 7.2 智能算法交易参数
在 `get_config` 中配置：
- `algo_trading`: 是否开启智能交易算法 (True/False)
- `order_timeout`: 订单超时时间(秒)，0表示不超时
- `retry_limit`: 撤单后尝试重发的最大次数
- `retry_offset_ticks`: 重发时的价格偏移跳数 (相对于对手价)

---

### 8. 常见问题 (FAQ)

### Q1: 找不到数据？

**A:** 检查：
- symbol代码是否正确（如`rb888`）
- 日期范围是否有效
- API账号密码是否在 `trading_config.py` 中配置

### Q2: 策略不执行？

**A:** 检查：
- 数据长度是否足够（MA20需要至少20根K线）
- 是否有早期返回（return）
- 添加 `api.log()` 调试

### Q3: 持仓不对？

**A:** 检查：
- `order_type` 是否正确
- 回测用 `'next_bar_open'`，实盘用 `'market'`
- 实盘检查 `order_offset_ticks` 设置

### Q4: 回测和实盘结果差异大？

**A:** 可能原因：
- `adjust_type` 复权类型不一致
- `slippage_ticks` 滑点设置不合理
- `price_tick` 最小变动价位设置错误

### Q5: SIMNOW连接失败？

**A:** 尝试：
- 更换服务器（电信1 → 电信2 → 移动）
- 检查账号密码是否正确
- 检查网络连接
- 工作日9:00-15:30测试

### Q6: 如何添加新账户？

**A:** 编辑 `ssquant/config/trading_config.py`：

```python
ACCOUNTS = {
    'simnow_default': {...},
    'my_new_account': {        # 添加新账户
        'investor_id': 'xxx',
        'password': 'xxx',
        # ...
    },
}
```

---

## 学习路径

### 初学者（第1周）

1. 阅读本手册
2. 运行 `examples/B_双均线策略.py`
3. 修改参数，观察结果
4. 尝试编写简单策略

### 进阶（第2-4周）

1. 学习多品种策略 `B_多品种多周期交易策略.py`
2. 尝试SIMNOW模拟
3. 研究高级示例（海龟、套利）
4. 参数优化 `B_多品种多周期交易策略_参数优化.py`

### 高级（第5周+）

1. TICK流策略 `C_纯Tick高频交易策略.py`
2. 机器学习策略 `B_机器学习策略_随机森林.py`
3. 实盘小资金测试
4. 正式部署

---

## 最佳实践

### 开发流程

```
策略构思 → 回测验证 → SIMNOW测试 → 小资金实盘 → 正式运行
   ↑_____________|（发现问题，优化策略）
```

### 风险控制

```python
def strategy_with_stoploss(api):
    close = api.get_close()
    pos = api.get_pos()
    
    # 止损：亏损5%
    entry_price = api.get_param('entry_price', 0)
    if pos > 0 and entry_price > 0:
        if close.iloc[-1] < entry_price * 0.95:
            api.sell(order_type='next_bar_open', reason='止损')
```

### 参数管理

```python
strategy_params = {
    'fast_ma': 5,
    'slow_ma': 20,
    'stop_loss': 0.05,
}

runner.run(
    strategy=my_strategy,
    strategy_params=strategy_params
)
```

---

## 下一步

- 查看 [API参考手册.md](API参考手册.md) 了解完整API
- 查看 [数据管理指南.md](数据管理指南.md) 了解数据获取
- 查看 `examples/` 学习示例策略

---

**祝你量化交易成功！** 📈
